 -- Following code will create three procedures
-- 1. load_emp_schema : To create tables and load data in it.
-- 2. clean_emp_schema : To drop all tables that we have created in schema
-- 3. drop_and_reload : To call clean_scott_schema and the refresh_scott.

DELIMITER #
CREATE PROCEDURE load_emp_schema()
BEGIN
...
DELIMITER #
CREATE PROCEDURE clean_emp_schema()
BEGIN
...
DELIMITER #
CREATE PROCEDURE drop_and_reload()
BEGIN
	call clean_emp_schema();
	call load_emp_schema();
    SELECT sysdate();
END#

call drop_and_reload();
